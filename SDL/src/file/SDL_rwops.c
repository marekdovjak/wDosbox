...
win32_file_open(SDL_RWops * context, const char *filename, const char *mode)
{
	...
	h = CreateFile2(filenameW, (w_right | r_right), (w_right) ? 0 : FILE_SHARE_READ, (must_exist | truncate | a_mode), NULL);
...
win32_file_seek(SDL_RWops * context, long offset, int whence)
{
	...
	LARGE_INTEGER lPos = { offset, 0 };
	if (SetFilePointerEx(context->hidden.win32io.h, lPos, NULL, win32whence))
		return offset;
                 ...
                 size_t num)
{
    ...
    if (context->hidden.win32io.buffer.left) {
		LARGE_INTEGER lPos = { -(LONG)context->hidden.win32io.buffer.left, 0 };
        SetFilePointerEx(context->hidden.win32io.h,
                       lPos, NULL,
                       FILE_CURRENT);
        context->hidden.win32io.buffer.left = 0;
    }
    ...
    if (context->hidden.win32io.append) {
        ...
        if (SetFilePointerEx(context->hidden.win32io.h, lPos, NULL, FILE_END) ==
